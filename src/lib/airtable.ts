...
export const getContacts = cache(async (): Promise<Contact[]> => {
  ...
  return rows.map((r) => ({
    ...
    website: str(r.fields["Website"]),
    taxId: str(r.fields["Tax ID"]),
    currency: str(r.fields["Currency"]) as Contact["currency"],
    paymentTerms: str(r.fields["Payment Terms"]) as Contact["paymentTerms"],
...
export const getProducts = cache(async (): Promise<Product[]> => {
  ...
  return rows.map((r) => ({
    ...
    hsCode: str(r.fields["HS Code"]),
    countryOfOrigin: str(r.fields["Country of Origin"]),
    weightKg: num(r.fields["Weight (kg)"]),
    notes: str(r.fields["Notes"]),
  }));
});
...
export const getDeals = cache(async (): Promise<Deal[]> => {
  ...
  return rows.map((r) => ({
    ...
    originCountry: str(r.fields["Origin Country"]),
    destinationCountry: str(r.fields["Destination Country"]),
    commissionRate: num(r.fields["Commission Rate"]),
    incoterm: str(r.fields["Incoterm"]) as Deal["incoterm"],
    paymentStatus: str(r.fields["Payment Status"]) as Deal["paymentStatus"],
    currency: str(r.fields["Currency"]) as Deal["currency"],
...
export const getShipments = cache(async (): Promise<Shipment[]> => {
  ...
  return rows.map((r) => ({
    ...
    vessel: str(r.fields["Vessel"]),
    containerNo: str(r.fields["Container No"]),
    billOfLading: str(r.fields["Bill of Lading"]),
    grossWeightKg: num(r.fields["Gross Weight (kg)"]),
    containerCount: num(r.fields["Container Count"]),
    containerType: str(r.fields["Container Type"]) as Shipment["containerType"],
    freightCost: num(r.fields["Freight Cost"]),
    incoterm: str(r.fields["Incoterm"]) as Shipment["incoterm"],
